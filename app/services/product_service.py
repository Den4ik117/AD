...
from app.schemas import ProductCreate, ProductMessage, ProductUpdate
...
class ProductService:
    ...
    async def create_product(self, payload: ProductCreate) -> Product:
        return await self._product_repository.create(**payload.model_dump())

    async def update_product(self, product_id: UUID, payload: ProductUpdate) -> Product:
        update_fields = payload.model_dump(exclude_unset=True)
        return await self._product_repository.update(product_id, **update_fields)

    async def mark_out_of_stock(self, product_id: UUID) -> Product:
        return await self._product_repository.update(product_id, stock_quantity=0)
